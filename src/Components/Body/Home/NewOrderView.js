...
import { AcceptOrder } from '../../APIs/PostAPIs';
...
class NewOrderView extends Component{
    ...
    constructor(props){
        ...
        this.SendAcceptOrderRequest = this.SendAcceptOrderRequest.bind(this);
    ...
    SendAcceptOrderRequest(){
        ...
        this.closeOrderAcceptModal()
        this.openSuccessModal();
        AcceptOrder(this.props.order.OrderID, this.props.order.OrderItemID, this.state.AcceptOrderViewData)
        .then((result) => {
                    ...
                    <div className="modal-dialog modal-lg" role="document">
                        <div className="modal-content">
                            ...
                            <div className="modal-footer">
                                <button type="button" className="btn btn-primary" onClick={ () => this.SendAcceptOrderRequest() } >Accept</button>
    ...
    getNewOrder(order,history){
        return(
            <div id={"OrderListView"+order.OrderID} key={order.OrderID} className="order-list-view">
                ...
                <Modal id="SuccessMessage" isOpen={this.state.SuccessMessageModal}>
                    <div className="modal-dialog modal-lg" role="document">
                        <div className="modal-content">
                            <div className="modal-header padding-bottom-zero">
                                <h4 className="modal-title"> <span className="glyphicon glyphicon-ok-sign order-view-accept-bg" /> Order Accept Request</h4>
                            </div>
                            <div className="modal-body bg-grey text-center">
                                <div className="circle-loader">
                                    <div className="checkmark draw"></div>
                                </div>

                                <div className="vp-accept-success-text"> 
                                    { this.state.IsOrderAccepted ? "Successfull" : "Request sent....." }
                                </div>
                            </div>
                            <div className="modal-footer">
                                { this.state.IsOrderAccepted  ? 
                                <button type="button" className="btn btn-default" onClick={ () => this.closeSuccessModal() } >Close</button>
                                :"" }
                            </div>
                        </div>
    ...
    render(){
        ...
        var returnObject = []
        return this.getNewOrder(order,this.props.history);
    }
...
export default withRouter(WithAppState(NewOrderView));
...
OrderItemNotation: 3484662.1
SubjectAddress: 6516+STICHTER+AVE+Dallas+Dallas%2C+TX+75230
ClientName: Nationstar+Credit+Risk+Forward
ClientAddress: 350+Highland+Dr.+Lewisville%2C+TX+75067
VendorFee: 11
LoanPurpose: Asset+Valuation
ProductName: SFR+1004
AssignedDateDisplay: 5%2F17%2F2018+at+8%3A48+AM
AcceptByDateDisplay: 5%2F18%2F2018+at+11%3A20+PM
CurrentOrderAction: New
VendorDueDateDisplay: 5%2F24%2F2018+at+5%3A00+PM
FHACaseNumber: 
SubjectAddressCityStateAndZip: 
FormModel.OrderID: 3484662
FormModel.OrderItemID: 1
FormModel.VendorID: 240202
FormModel.AllowConditionalAcceptance: True
FormModel.VendorDueDate: 5%2F24%2F2018+5%3A00%3A00+PM
DoesVendorProposeDates: True
